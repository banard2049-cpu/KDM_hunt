...
布场结果会推送到同一局域网内的平板／电视／电脑上只读显示，地址固定为 `http://<局域网IP>:<端口>/d/`（同时显示二维码），旧的长地址 `/display/<token>/` 仍然有效。主控端可调整版图与卡牌大小、显示／隐藏初始位置、交换版图与卡牌、旋转卡牌区、对齐方式，以及切换显示决战版图或该等级的 Boss 规则书。分享只提供只读快照，不暴露控制接口，主控写入仅限本机回环访问。Android 端由原生 `ShowdownHost` 插件直接开服务器，不需要 Python 后端。
