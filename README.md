...
「第二屏幕」按钮支持 **45 个 Boss／变体、137 个等级、21 个地形扩展、63 张地形卡** 的决战（Showdown）布场。每个等级按规则拆出固定地形、指定地形与随机地形数量，先取出规则指定的地形，再从随机牌池补齐；牌池不足时明确报错，不会少发或重复。随机地形池默认只含基础牌库，可按扩展分组、按名称搜索或单张勾选；粉丝扩地形默认不入池，勾选「显示粉丝扩地形」后加入，若所选等级规则本身需要该地形则自动带入。
