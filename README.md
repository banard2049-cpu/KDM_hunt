...
顶部的“战利品”入口支持 31 个官方 Boss／变体、92 个等级，包含基础游戏、赌博者宝箱、12 扩与其他官方扩展。狩猎遭遇会预选对应 Boss 与等级；获胜后点击“抽取战后奖励”。非狩猎战斗可手动选择。

卡牌按实例逐张展示，可点击放大、弃置和撤销。弃置不回牌库，同场不能再次抽到或指定拿取该实例；同名的其他实例不受影响。随机补抽、指定拿牌与标准奖励共用剩余牌库，牌库不足时不洗回、不部分发奖。标准奖励每场一次，领取后不能通过撤销后续操作再次领取。

战利品进度保存于浏览器 `kdm-loot-official-v1`，决战使用 `kdm-showdown-official-v1`，狩猎使用 `kdm-current-official-v1`。纯官方版使用独立进度，避免旧版抽牌序号在删牌后错位；旧版存档保留原样，不自动载入。
...
「第二屏幕」按钮支持 **31 个 Boss／变体、92 个等级、12 个地形扩展、41 张地形卡** 的决战（Showdown）布场。每个等级按规则拆出固定地形、指定地形与随机地形数量，先取出规则指定的地形，再从随机牌池补齐；牌池不足时明确报错，不会少发或重复。随机地形池默认只含基础牌库，可按扩展分组、按名称搜索或单张勾选。

布场结果会推送到同一局域网内的平板／电视／电脑上只读显示，地址固定为 `http://<局域网IP>:<端口>/d/`，第二屏幕打开后这个地址会显示在左侧栏「第二屏幕控制」里战斗信息（Boss · 等级 · 随机地形张数）的下方，直接在平板／电视的浏览器里输入即可，旧的长地址 `/display/<token>/` 仍然有效。主控端可调整版图与卡牌大小、显示／隐藏初始位置、交换版图与卡牌、旋转卡牌区、对齐方式，以及切换显示决战版图或该等级的 Boss 规则书。分享只提供只读快照，不暴露控制接口，主控写入仅限本机回环访问。Android 端由原生 `ShowdownHost` 插件直接开服务器，不需要 Python 后端。
...
本分支移除社区扩展 Boss、追加狩猎牌、资源与地形。Bone Eaters 属于赌博者宝箱，保留并修正分类。王之侍卫恢复基础规则奖励表。

GitHub Release 上的原始图包保持不变。下载后运行 `npm run assets:clean`（需要 Python 和 Pillow），按 `scripts/official-assets.json` 生成只保留所需图块的图集并删除本地未引用资源。`npm run web:stage` 自动执行清理并重建离线目录，旧图包不会把社区内容带入新版安装包。
